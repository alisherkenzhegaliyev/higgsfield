import { r as __exportAll } from "./react-BH0yRK75.js";
//#region node_modules/tldraw/dist-esm/lib/utils/svg/sanitizeSvg.mjs
var sanitizeSvg_exports = /* @__PURE__ */ __exportAll({ sanitizeSvg: () => sanitizeSvg });
/*!
...
function sanitizeCssValue(css) {
	...
	decoded = decoded.replace(/@import\s+(?:url\s*\([^)]*\)|"[^"]*"|'[^']*')[^;]*;?|@import\b[^;]*;?/gi, "");
	...
	decoded = decoded.replace(/url\s*\(\s*(['"]?)(.*?)\1\s*\)/gis, (match, _quote, uri) => {
		...
		if (SAFE_CSS_DATA_MIME.test(stripped) || FRAGMENT_REF.test(stripped)) return match;
		return "";
	});
	return decoded;
}
...
var ANIMATION_TAGS = /* @__PURE__ */ new Set([
	"animate",
	"set",
	"animatecolor",
	"animatetransform"
...
function sanitizeSvgAttributes(el, depth) {
	for (let i = el.attributes.length - 1; i >= 0; i--) {
		...
		if (DATA_ATTR_PATTERN.test(name) || ARIA_ATTR_PATTERN.test(name)) continue;
		...
		if (name === "href" || name === "xlink:href") {
			...
			if (sanitized === null) el.removeAttribute(attr.name);
			else if (sanitized !== attr.value) attr.value = sanitized;
			continue;
		}
		if (name === "style") {
		...
		if (URL_BEARING_SVG_ATTRS.has(name) && /url\s*\(/i.test(attr.value)) attr.value = sanitizeCssValue(attr.value);
	}
}
function sanitizeHtmlAttributes(el) {
	...
	for (let i = el.attributes.length - 1; i >= 0; i--) {
		...
		if (name === "href") {
			...
			if (!SAFE_LINK_PROTOCOLS.test(stripped)) el.removeAttribute(attr.name);
			continue;
		}
		if (name === "style") attr.value = sanitizeCssValue(attr.value);
	}
}
function sanitizeNode(node, mode, depth) {
	for (let i = node.children.length - 1; i >= 0; i--) {
		...
		if (mode === "svg") if (tag === "foreignobject") {
			sanitizeSvgAttributes(child, depth);
			sanitizeNode(child, "html", depth);
		} else if (tag === "style") {
			sanitizeSvgAttributes(child, depth);
			if (child.textContent) child.textContent = sanitizeStyleElement(child.textContent);
		} else if (ANIMATION_TAGS.has(tag) && isAnimationDangerous(child)) child.remove();
		else if (ALLOWED_SVG_TAGS.has(tag)) {
			sanitizeSvgAttributes(child, depth);
			sanitizeNode(child, "svg", depth);
		} else child.remove();
		else if (BLOCKED_HTML_TAGS.has(tag)) child.remove();
...
//#endregion
export { sanitizeSvg_exports as n, sanitizeSvg as t };

//# sourceMappingURL=sanitizeSvg-v-hB6LDI.js.map